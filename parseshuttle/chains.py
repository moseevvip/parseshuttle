...
# @author ¯\\_(ツ)_/¯ El_Gloton ¯\\_(ツ)_/¯ 
 # @file Description
 # @desc Created on 2020-03-13 1:02:06 pm
 # @copyright APPI SASU
 #
...
class Link:
    ...
    def __str__(self):
        ...
        if self.kwargs:
            args_reprs.extend(f'{name}={repr(value)}' for name, value in self.kwargs.items())
        if args_reprs:
...
class Ops:
    ...
    # Microdata
    @has_args
    def itemscope(self, name):
        return C.css(f'[itemscope][itemprop*={name}]')

    @has_args
    def itemprop(self, name):
        return C.css(f'[itemprop*={name}]')

    @has_args
    def microdata(self, name):
